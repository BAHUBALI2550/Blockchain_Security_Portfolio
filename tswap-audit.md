...
# Executive Summary
This audit is for TSwapPool. The severity is of High, Medium and Low.
## Issues found
...
**Proof of Code:**
To test, include this code in `TSwapPool.t.sol` :

```javascript
function testIncorrectSwapExactOutput() public {
        uint256 initialLiquidity = 50e18;
        ...
        poolToken.mint(user1, userInitialPoolTokenBalance);
        vm.startPrank(user1);
        poolToken.approve(address(pool), type(uint256).max);
        pool.swapExactOutput(poolToken, weth, 1 ether, uint64(block.timestamp));
        assertLt(poolToken.balanceOf(user1), 1 ether);
        vm.stopPrank();
...
**Proof of Concept:** 
1. The price of 1 weth is 10 usdc.
2. user wants to convert 1 weth for 10 usdc
3. user inputs a `swapExactOutput` for 10 usdc
4. As the transaction goes through he gets charged 10 weth for just 10 usdc instead of 1 weth
